...
switch ($action) {
    case 'setflag':
        ...
        tep_redirect(tep_href_link(FILENAME_BANNER_MANAGER, tep_get_all_get_params(['action', 'flag'])));
    break;
    
    case 'insert':
    case 'update':
        if (isset($_POST['banners_id'])) $banners_id = xprios_prepare_post('banners_id');
        $banners_title = xprios_prepare_post('banners_title');
        $banners_url = xprios_prepare_post('banners_url');
        $new_banners_group = xprios_prepare_post('new_banners_group');
        $banners_group = (empty($new_banners_group)) ? xprios_prepare_post('banners_group') : $new_banners_group;
        $banners_html_text = xprios_prepare_post('banners_html_text', true);
        $banners_image_local = xprios_prepare_post('banners_image_local');
        $banners_image_target = xprios_prepare_post('banners_image_target');
        $db_image_location = '';
        $expires_date = xprios_prepare_post('expires_date');
        $expires_impressions = xprios_prepare_post('expires_impressions');
        $date_scheduled = xprios_prepare_post('date_scheduled');
        ...
        if (empty($banners_html_text) && $banners_group!='footer') {
            if (empty($banners_image_local)) {
                $banners_image = new Upload('banners_image', DIR_FS_CATALOG_IMAGES . $banners_image_target);
        ...
        if ($banner_error == false) {
            ...
            if (tep_not_null($date_scheduled)) {
                ...
                tep_db_query("update " 
                    . TABLE_BANNERS . " "
                    . "set "
                    . "status = '0', date_scheduled = '" . tep_db_input($date_scheduled) . "', "
    ...
    case 'deleteconfirm':
        $banners_id = xprios_prepare_get('bID');
        ...
        tep_redirect(tep_href_link(FILENAME_BANNER_MANAGER, isset($_GET['page']) ? 'page=' . $_GET['page'] : ''));
    break;
}
...
<?php
    ...
    while ($banners = tep_db_fetch_array($banners_query)) {
      ...
      if (isset($bInfo) && is_object($bInfo) && ($banners['banners_id'] == $bInfo->banners_id)) {
        echo '              <tr id="defaultSelected" class="dataTableRowSelected" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_BANNER_STATISTICS, 'page=' . $page . '&bID=' . $bInfo->banners_id) . '\'">' . "\n";
      } else {
        echo '              <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)" onclick="document.location.href=\'' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $banners['banners_id']) . '\'">' . "\n";
      }
?>
...
<?php
      if ($banners['status'] == '1') {
        echo tep_image(DIR_WS_IMAGES . 'icon_status_green.gif', 'Active', 10, 10) . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=0') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_red_light.gif', 'Set Inactive', 10, 10) . '</a>';
      } else {
        echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $banners['banners_id'] . '&action=setflag&flag=1') . '">' . tep_image(DIR_WS_IMAGES . 'icon_status_green_light.gif', 'Set Active', 10, 10) . '</a>&nbsp;&nbsp;' . tep_image(DIR_WS_IMAGES . 'icon_status_red.gif', 'Inactive', 10, 10);
      }
?></td>
                <td class="dataTableContent" align="right"><?php 
        echo '<a href="' . tep_href_link(FILENAME_BANNER_STATISTICS, 'page=' . $page . '&bID=' . $banners['banners_id']) . '">' . tep_image(DIR_WS_ICONS . 'statistics.gif', ICON_STATISTICS) . '</a>&nbsp;'; 
        if (isset($bInfo) && is_object($bInfo) && ($banners['banners_id'] == $bInfo->banners_id)) { 
            echo tep_image(DIR_WS_IMAGES . 'icon_arrow_right.gif', ''); 
        } else { 
            echo '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $banners['banners_id']) . '">' . tep_image(DIR_WS_IMAGES . 'icon_info.gif', IMAGE_ICON_INFO) . '</a>'; 
        } ?>&nbsp;</td>
              </tr>
<?php
    }
...
<?php
    ...
    switch ($action) {
        ...
        $contents = array('form' => tep_draw_form('banners', FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $bInfo->banners_id . '&action=deleteconfirm'));
        ...
        $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $_GET['bID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;
    default:
        if (is_object($bInfo)) {
            $heading[] = array('text' => '<b>' . $bInfo->banners_title . '</b>');

            $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $bInfo->banners_id . '&action=new') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_BANNER_MANAGER, 'page=' . $page . '&bID=' . $bInfo->banners_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
