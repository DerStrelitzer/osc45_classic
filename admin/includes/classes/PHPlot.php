...
class PHPlot{
    ...
    private 
        ...
        $img = null,
        $i_dark = [],
        $ndx_plot_bg_color = [0, 0, 0],
        $ndx_bg_color = [0, 0, 0],
        $ndx_label_color = [0, 0, 0],
        $ndx_grid_color = [0, 0, 0], 
        $ndx_light_grid_color = [0, 0, 0],
        $ndx_tick_color = [0, 0, 0],
        $ndx_title_color = [0, 0, 0],
        $ndx_i_light = [0, 0, 0],
        $ndx_i_dark = [0, 0, 0],
        $ndx_error_bar_color = [0, 0, 0],
        $ndx_data_border_color = [0, 0, 0],
        $ndx_text_color = [0, 0, 0],
        $ndx_data_color = [0, 0, 0],
        $plot_area_width = 0,
        $plot_area_height = 0,
        $min_x = 0,
        $x_label_height = 0,
...
//DataAxis Labels (on each axis)
    ...
    public $x_time_format = "H:i:s";    //See http://www.php.net/manual/html/function.strftime.html
...
*************************************** */
    ...
    if ($this->use_ttf == 1 ) { 
			$size = $this->TTFBBoxSize($which_size, $which_angle, $which_size, $which_text); 
			if ($which_valign == 'bottom') { 
				$which_ypos = $which_ypos + ImageFontHeight($which_size);
			}
			if ($which_halign == 'center') { 
				$which_xpos = $which_xpos - $size[0]/2;
			}
			ImageTTFText($this->img, $which_size, $which_angle, $which_xpos, $which_ypos, $which_color, $which_size, $which_text); 
		} else { 
			if ($which_valign == 'top') { 
				$which_ypos = $which_ypos - ImageFontHeight($which_size);
			}
            ...
            $str = preg_split("/\n/", $which_text, -1, PREG_SPLIT_NO_EMPTY); 
			$height = ImageFontHeight($which_size);
			$width = ImageFontWidth($which_size);
			if ($which_angle == 90) {  //Vertical Code Submitted by Marlin Viss
				for($i=0;$i<count($str);$i++) { 
					ImageStringUp($this->img, $which_size, ($i*$height + $which_xpos), $which_ypos, $str[$i], $which_color);
				} 
			} else {
				for($i=0;$i<count($str);$i++) { 
					if ($which_halign == 'center') { 
                    	$xpos = intval($which_xpos - strlen($str[$i]) * $width/2);
 						ImageString($this->img, $which_size, $xpos, ($i*$height + $which_ypos), $str[$i], $which_color);
					} else { 
						ImageString($this->img, $which_size, $which_xpos, ($i*$height + $which_ypos), $str[$i], $which_color); 
					}
				} 
			}
	...
	public function SetPlotType($which_pt) {
		...
		$asked = trim($which_pt);
		if (preg_match('/' . preg_quote($asked, '/') . '/i', $accepted)) {
	...
	public function SetMargins() {
		...
		$str = preg_split("/\n/", $this->title_txt, -1, PREG_SPLIT_NO_EMPTY); 
		$nbLines = count($str); 
...
//ajo working
		...
		ImageString($this->img, $this->small_font, round($yaxis_x - $this->y_label_width - $this->tick_length/2),
			round( -($this->small_font_height/2.0) + $y_pixels),$which_ylab, $this->ndx_text_color);
	}
		...
		foreach ($this->data_values as $j => $row) {
			...
			foreach ($row as $k => $v) {
				if ($k == 0) {
					...
					// Draw Bars ($v)
					$x1 = round($x_now - $this->data_group_space + ($k-1)*$this->record_bar_width);
					$x2 = round($x1 + $this->record_bar_width*$this->bar_width_adjust); 
