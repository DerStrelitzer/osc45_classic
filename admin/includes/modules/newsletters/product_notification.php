...
//--></script>';
        ...
        $choose_audience_string .= '<form name="notifications" action="' . tep_href_link(FILENAME_NEWSLETTERS, 'page=' . $_GET['page'] . '&nID=' . $_GET['nID'] . '&action=confirm') . '" method="post" onSubmit="return selectAll(\'notifications\', \'chosen[]\')"><table border="0" width="100%" cellspacing="0" cellpadding="2">' . "\n" .
                                 ...
                                 '    <td align="center" class="main"><b>' . TEXT_SELECTED_PRODUCTS . '</b><br />' . tep_draw_pull_down_menu('chosen[]', [], '', 'size="20" style="width: 20em;" multiple') . '</td>' . "\n" .
                                 '  </tr>' . "\n" .
                                 '</table></form>';
    ...
    function send($newsletter_id)
    {
        ...
        tep_db_query("update " . TABLE_NEWSLETTERS . " set status = '1', date_sent = now() where newsletters_id = '" . tep_db_input($newsletter_id) . "'");
    }
}
