...
// decide if the value in bar is a color code or image.
        if (preg_match('/^#/', $bars[$i])) {
            ...
            $horizontal_graph_string .= '<img src="' . $bars[$i] . '" height="10" width="' . ($values[$i] * $vals['scale']) . '" alt="" border="0" />';
        }
...
// is set at 1 pixel if the value is zero. - Jan Diepens
        ...
        $vertical_graph_string .= '" alt="" border="0" /></td>' . "\n";
    } // endfor
...
// decide if the value in bar is a color code or image.
      if (preg_match('/^#/', $bars[$i])) {
        ...
        $double_horizontal_graph_string .= '<img src="' . $bars[$i] . '" height="10" width="' . ($values[$i] * $vals['scale']) . '" border="0" alt="" />';
      }
...
// if a background was choosen don't print cell BGCOLOR
      ...
      $double_vertical_graph_string .= '" border="0" alt="" /></td>' . "\n" .
                                       '        <td align="center" valign="bottom"';
...
function tep_banner_graph_monthly($banner_id)
{
    ...
    for ($i=1; $i<13; $i++) {
        $names[] = xprios_date_month_name($i);
        $values[] = '0';
        $dvalues[] = '0';
    }

    $banner_stats_query = tep_db_query("select month(banners_history_date) as banner_month, sum(banners_shown) as value, sum(banners_clicked) as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . (int)$banner_id . "' and year(banners_history_date) = '" . $year . "' group by month(banners_history_date)");
    while ($banner_stats = tep_db_fetch_array($banner_stats_query)) {
        $names[($banner_stats['banner_month']-1)] = xprios_date_month_name($banner_stats['banner_month']);
...
// draws a double vertical bar graph for the banner views vs clicks statistics
  function tep_banner_graph_daily($banner_id) {
    ...
    $banner_stats_query = tep_db_query("select dayofmonth(banners_history_date) as banner_day, banners_shown as value, banners_clicked as dvalue from " . TABLE_BANNERS_HISTORY . " where banners_id = '" . (int)$banner_id . "' and month(banners_history_date) = '" . $month . "' and year(banners_history_date) = '" . $year . "'");
    while ($banner_stats = tep_db_fetch_array($banner_stats_query)) {
    ...
    $graph_vals = @array('vlabel'=>TEXT_BANNERS_DATA,
                        'hlabel'=>sprintf(TEXT_BANNERS_DAILY_STATISTICS, $banner['banners_title'], xprios_date_month_name($month), $year),
                        'type'=>'3',
                        'cellpadding'=>'',
                        'cellspacing'=>'1',
                        'border'=>'',
