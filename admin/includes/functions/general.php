...
function xprios_prepare_post($index='', $html=false)
{
    $value = '';
...
function xprios_prepare_get($index='', $html=false)
{
    $value = '';
    $html = $html===true ? true:false;
...
function xprios_prepare_input($string='', $html=false)
{
    $html = $html===true ? true:false;
    if (is_string($string)) {
...
function xprios_date_month_name($month = '', $short=false) : string
{
    $return = $month;
    if ($short != false) {
        $reverse = [
        ...
        $names = preg_split('/[,]/', DATE_MONTH_NAMES_SHORT, -1, PREG_SPLIT_NO_EMPTY);
    } else {
        $reverse = [
            'january' => 1,
            'february' => 2,
        ...
        $names = preg_split('/[,]/', DATE_MONTH_NAMES, -1, PREG_SPLIT_NO_EMPTY);
    }
    if (is_numeric($month) && isset($names[intval($month)-1])) {
        $return = $names[intval($month)-1];
    } else {
        $key = trim(strtolower($month));
        if (isset($revers[$key]) && isset($names[$key - 1])) {
            $return = $names[$key - 1];
        }
    }
    return $return;
}
function xprios_date_day_name($month = '', $short=false) : string
{
    $return = $month;
    if ($short != false) {
        $reverse = [
        ...
        $names = preg_split('/[,]/', DATE_DAY_NAMES_SHORT, -1, PREG_SPLIT_NO_EMPTY);
    } else {
        $reverse = [
            'monday' => 1,
            'tuesday' => 2,
        ...
        $names = preg_split('/[,]/', DATE_DAY_NAMES, -1, PREG_SPLIT_NO_EMPTY);
    }
    if (is_numeric($month) && isset($names[intval($month)-1])) {
        $return = $names[intval($month)-1];
    } else {
...
//
function ingo_delete_a_image($image_name='') {
  $duplicate_image_query = tep_db_query("select count(*) as total from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_IMAGES . " pi where p.products_image = '" . tep_db_input($image_name) . "' or pi.image_name = '" . tep_db_input($image_name) . "'");
  if (tep_db_num_rows($duplicate_image_query) < 2) {
    if (file_exists(DIR_FS_CATALOG_IMAGES . $image_name)) {
      @unlink(DIR_FS_CATALOG_IMAGES . $image_name);
    }
    if (file_exists(DIR_FS_CATALOG .'thumbs/' . $image_name)) {
      @unlink(DIR_FS_CATALOG . 'thumbs/' . $image_name);
    }
  }
}
...
////
// Get the Categories description
...
function tep_sanitize_string($string, $html = false)
{
  ...
  function tep_get_all_get_params($exclude_array = '') {
    ...
    if ($exclude_array == '') $exclude_array = [];
...
function tep_date_long($raw_date)
{
    ...
    return date('w', $time) . ', ' . date('d', $time) . '. ' . xprios_date_month_name('n', $time) . ' ' . date('Y', $time);
}
...
function tep_get_system_information()
{
    ...
    $execute = (string)@exec('uname -a');
    $shk = preg_split('/[\\s,]+/', $execute, 5, PREG_SPLIT_NO_EMPTY);

    if (isset($shk[0]) && $shk[0] != '') {
        $system = $shk[0] ;
    } else {
        $_env = getenv();
        $os = str_replace(['_'], ' ', $_env['OS']);
        if ($os != '') {
            $system = $os;
        } else {
    ...
    $host   = isset($shk[1]) ? $shk[1] : 'localhost';
    $kernel = isset($shk[2]) ? $shk[2] : '(unknown kernel)';
    ...
    return [
        ...
        'php'         => PHP_VERSION,
        'zend'        => function_exists('zend_version') ? zend_version() : '(unknown Zend)',
        'db_server'   => DB_SERVER,
...
// Ingo Beginn images unlimited
    $add_images_query = tep_db_query("select image_id, image_name from " . TABLE_PRODUCTS_IMAGES . " where products_id = '" . (int)$product_id . "'");
    while ($add_images_result = tep_db_fetch_array($add_images_query)) {
      tep_db_query("delete from " . TABLE_PRODUCTS_IMAGES . " where image_id = '" . $add_images_result['image_id'] . "'");
      ingo_delete_a_image($add_images_result['image_name']);
    }
// Ingo Beginn images unlimited
  ...
  function tep_remove_order($order_id, $restock = false) {
    ...
    tep_db_query("delete from " . TABLE_BANKTRANSFER . " where orders_id = '" . (int)$order_id . "'");
  }
...
// TABLES: tax_rates
 ...
 // Ingo: Versuch --  auskommentiert, damit nicht alle Zonen addiert werden (16 + 16 + 16 ...) prozent
 //  $tax_query = tep_db_query("select SUM(tax_rate) as tax_rate from " . TABLE_TAX_RATES . " where tax_class_id = '" . (int)$class_id . "' group by tax_priority"); // original!
  $tax_query = tep_db_query("select tax_rate from " . TABLE_TAX_RATES . " where tax_class_id = '" . (int)$class_id . "' group by tax_priority"); //ingo
