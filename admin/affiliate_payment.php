<?php
/*
  $Id: affiliate_payment.php,v 1.18 2003/07/12 13:03:21 simarilius Exp $
...
$payments_status_query = tep_db_query("select affiliate_payment_status_id, affiliate_payment_status_name from " . TABLE_AFFILIATE_PAYMENT_STATUS . " where affiliate_language_id = '" . (int)$_SESSION['languages_id'] . "'");
...
// Select all affiliates who earned enough money since last payment
        $sql = "SELECT "
            ...
            . "WHERE "
            . "a.affiliate_billing_status != 1 and a.affiliate_orders_id = o.orders_id "
            . "and o.orders_status >= " . AFFILIATE_PAYMENT_ORDER_MIN_STATUS . " and a.affiliate_date <= '" . $oldday . "' "
...
// Get all orders which are AFFILIATE_BILLING_TIME days old
            $sql = "SELECT a.affiliate_orders_id "
                . "FROM " . TABLE_AFFILIATE_SALES . " a, " . TABLE_ORDERS . " o "
                . "WHERE a.affiliate_billing_status!=1 and a.affiliate_orders_id=o.orders_id "
...
// Get affiliate Informations
            $sql = "SELECT "
                . "a.*, c.countries_id, c.countries_name, c.countries_iso_code_2, c.countries_iso_code_3, c.address_format_id "
...
// Notify Affiliate
    ...
    case 'update_payment':
        ...
        if ($check_status['affiliate_payment_status'] != $status) {
            tep_db_query("update " . TABLE_AFFILIATE_PAYMENT . " set affiliate_payment_status = '" . tep_db_input($status) . "' where affiliate_payment_id = '" . tep_db_input($pID) . "'");
...
// Notify Affiliate
            ...
            tep_db_query("insert into " . TABLE_AFFILIATE_PAYMENT_STATUS_HISTORY . " (affiliate_payment_id, affiliate_new_value, affiliate_old_value, affiliate_notified) values ('" . tep_db_input($pID) . "', '" . tep_db_input($status) . "', '" . $check_status['affiliate_payment_status'] . "', '" . $affiliate_notified . "')");
            $order_updated = true;
        }
...
if ($action == 'edit' && isset($_GET['pID']) && tep_not_null($_GET['pID']) ) {
    $pID = xprios_prepare_get('pID');
...
<?php
if ($action == 'edit' && isset($payments_exists) && $payments_exists == true) {
      ...
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            ...
            <td class="pageHeading"><?php echo '<a href="' . tep_href_link(FILENAME_AFFILIATE_PAYMENT, 'action=start_billing' ) . '">' . tep_image_button('button_affiliate_billing.gif', IMAGE_AFFILIATE_BILLING) . '</a>'; ?></td>
            <td class="pageHeading" align="right"><?php echo tep_draw_separator('pixel_trans.gif', 1, HEADING_IMAGE_HEIGHT); ?></td>
...
<?php
    if (isset($_GET['sID'])) {
      ...
      $sID = xprios_prepare_get('sID');
      $payments_query_raw = "select p.* , s.affiliate_payment_status_name from " . TABLE_AFFILIATE_PAYMENT . " p , " . TABLE_AFFILIATE_PAYMENT_STATUS . " s where p.affiliate_payment_id = '" . tep_db_input($sID) . "' and p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '" . (int)$_SESSION['languages_id'] . "' order by p.affiliate_payment_id DESC";
    } elseif (isset($_GET['status']) && is_numeric($_GET['status'])) {
      $status = xprios_prepare_get('status');
      $payments_query_raw = "select p.* , s.affiliate_payment_status_name from " . TABLE_AFFILIATE_PAYMENT . " p , " . TABLE_AFFILIATE_PAYMENT_STATUS . " s where s.affiliate_payment_status_id = '" . tep_db_input($status) . "' and p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '" . (int)$_SESSION['languages_id'] . "' order by p.affiliate_payment_id DESC";
    } else {
      $payments_query_raw = "select p.* , s.affiliate_payment_status_name from " . TABLE_AFFILIATE_PAYMENT . " p , " . TABLE_AFFILIATE_PAYMENT_STATUS . " s where p.affiliate_payment_status = s.affiliate_payment_status_id and s.affiliate_language_id = '" . (int)$_SESSION['languages_id'] . "' order by p.affiliate_payment_id DESC";
    }
    $page = max(1, isset($_GET['page']) ? $_GET['page'] : '1');
    $payments_split = new SplitPageResults($page, MAX_DISPLAY_SEARCH_RESULTS, $payments_query_raw, $payments_query_numrows);
...
?>
              <tr>
                <td colspan="6"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $payments_split->display_count($payments_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $page, TEXT_DISPLAY_NUMBER_OF_PAYMENTS); ?></td>
                    <td class="smallText" align="right"><?php echo $payments_split->display_links($payments_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $page, tep_get_all_get_params(array('page', 'pID', 'action'))); ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
