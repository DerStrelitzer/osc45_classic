...
switch ($action) {
    case 'update_order':
        ...
        $status = intval(xprios_prepare_post('status'));
        ...
        if ($check_status['orders_status'] != $status || tep_not_null($comments)) {
            ...
            tep_db_query("insert into " . TABLE_ORDERS_STATUS_HISTORY . " (orders_id, orders_status_id, customer_notified, comments) values ('" . (int)$oID . "', '" . tep_db_input($status) . "', '" . tep_db_input($customer_notified) . "', '" . tep_db_input($comments)  . "')");
...
// Ingo
    $payment_text='';
    if (is_file(DIR_FS_CATALOG_MODULES . '/payment/' . $order->info['payment_class'] . '.php')) {
        require_once (DIR_FS_CATALOG_LANGUAGES . $_SESSION['language'] . '/modules/payment/' . $order->info['payment_class'] . '.php');
        require_once (DIR_FS_CATALOG_MODULES . '/payment/' . $order->info['payment_class'] . '.php');
        $payment_module = new $order->info['payment_class'];
        if ($data = $payment_module->get_data($oID)) {

            if ($data['sepa_fax'] == 1) {
                $payment_text = ', <b>Daten kommen per Faxformular!</b>';
            } else {
                $payment_text = '<table border="0" cellspacing="0" cellpadding="2" class="main">
                          <tr><td><b>Name:</b></td><td>' . $data['sepa_owner'] . '</td></tr>
                          <tr><td><b>Konto:</b></td><td>' . $data['sepa_iban'] . '</td></tr>
                          <tr><td><b>Blz:</b></td><td>' . $data['sepa_bic'] . '</td></tr></table>';
            }
        }
    }
// Ingo Ende
...
?>
      <tr>
        <td width="100%"><table border="0" width="100%" cellspacing="0" cellpadding="0">
          <tr>
            <td class="pageHeading"><?php echo HEADING_TITLE . ' #' . $oID . (($order->customer['id']=='0')? ' <b>no account!</b>':''); ?></td>
      ...
      <tr>
        <td><table border="0" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main" valign="top"><b><?php echo ENTRY_PAYMENT_METHOD; ?></b></td>
            <td class="main"><?php echo $order->info['payment_method'] . $payment_text; ?></td>
          </tr>
<?php
/*
    ...
    if (tep_db_num_rows($orders_history_query)) {
        while ($orders_history = tep_db_fetch_array($orders_history_query)) {
            ...
            echo '            <td class="smallText">' . $orders_status_array[$orders_history['orders_status_id']] . '</td>' . "\n"
                . '            <td class="smallText">' . (isset($orders_history['comments']) ? nl2br(tep_db_output($orders_history['comments'])) : '') . '&nbsp;</td>' . "\n"
                . '          </tr>' . "\n";
        }
    } else {
        echo '          <tr>' . "\n"
...
<?php
    if (isset($_GET['cID'])) {
        ...
        $status = intval(xprios_prepare_get('status'));
...
?>
                ...
                <td class="dataTableContent" align="center"><?php echo $orders['date_purchased']; ?></td>
...
?>
              <tr>
                <td colspan="5"><table border="0" width="100%" cellspacing="0" cellpadding="2">
                  <tr>
                    <td class="smallText" valign="top"><?php echo $orders_split->display_count($orders_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, $page, TEXT_DISPLAY_NUMBER_OF_ORDERS); ?></td>
                    <td class="smallText" align="right"><?php echo $orders_split->display_links($orders_query_numrows, MAX_DISPLAY_SEARCH_RESULTS, MAX_DISPLAY_PAGE_LINKS, $page, tep_get_all_get_params(array('page', 'oID', 'action'))); ?></td>
                  </tr>
                </table></td>
              </tr>
            </table></td>
