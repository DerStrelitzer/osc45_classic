<?php
/*
  admin/products_attributes.php by Ingo <www.strelitzer.de>
  based upon:
...
if (isset($_GET['option_page'])) $option_page = max(1, intval($_GET['option_page']));
if (isset($_GET['value_page'])) $value_page = max(1, intval($_GET['value_page']));
if (isset($_GET['attribute_page'])) $attribute_page = max(1, intval($_GET['attribute_page']));
...
if ($action != '') {
    ...
    switch ($action) {
      case 'add_product_options':
        $products_options_id = intval(xprios_prepare_post('products_options_id'));
        $option_name_array = xprios_prepare_post('option_name');

        for ($i=0, $n=sizeof($languages); $i<$n; $i ++) {
          $option_name = isset($option_name_array[$languages[$i]['id']]) ? $option_name_array[$languages[$i]['id']] : '';
          tep_db_query("insert into " . TABLE_PRODUCTS_OPTIONS . " (products_options_id, products_options_name, language_id) values ('" . $products_options_id . "', '" . tep_db_input($option_name) . "', '" . (int)$languages[$i]['id'] . "')");
        }
        tep_redirect(tep_href_link(FILENAME_PRODUCTS_ATTRIBUTES, $page_info));
        break;
      case 'add_product_option_values':
        $value_name_array = xprios_prepare_post('value_name');
        $value_id  = intval(xprios_prepare_post('value_id'));
        $option_id = intval(xprios_prepare_post('option_id'));

        for ($i=0, $n=sizeof($languages); $i<$n; $i ++) {
          $value_name = isset($value_name_array[$languages[$i]['id']]) ? $value_name_array[$languages[$i]['id']] : '';

          tep_db_query("insert into " . TABLE_PRODUCTS_OPTIONS_VALUES . " (products_options_values_id, language_id, products_options_values_name) values ('" . $value_id . "', '" . (int)$languages[$i]['id'] . "', '" . tep_db_input($value_name) . "')");
        }

        tep_db_query("insert into " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . " (products_options_id, products_options_values_id) values ('" . $option_id . "', '" . $value_id . "')");

        tep_redirect(tep_href_link(FILENAME_PRODUCTS_ATTRIBUTES, $page_info));
        break;
      case 'add_product_attributes':
        $products_id = intval(xprios_prepare_post('products_id'));
        $options_id = intval(xprios_prepare_post('options_id'));
        $values_id = intval(xprios_prepare_post('values_id'));
        $value_price = max(0, floatval(str_replace(',', '.', xprios_prepare_post('value_price'))));
        $price_prefix = xprios_prepare_post('price_prefix');

        tep_db_query("insert into " . TABLE_PRODUCTS_ATTRIBUTES . "(products_id, options_id, options_values_id, options_values_price, price_prefix) values ('" . $products_id . "', '" . $options_id . "', '" . $values_id . "', '" . tep_db_input($value_price) . "', '" . tep_db_input($price_prefix) . "')");

        if (DOWNLOAD_ENABLED == 'true') {
          $products_attributes_id = tep_db_insert_id();

          $products_attributes_filename = xprios_prepare_post('products_attributes_filename');
          $products_attributes_maxdays = max(1, intval(xprios_prepare_post('products_attributes_maxdays')));
          $products_attributes_maxcount = max(1, intval(xprios_prepare_post('products_attributes_maxcount')));

          if (tep_not_null($products_attributes_filename)) {
            tep_db_query("insert into " . TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " values (" . (int)$products_attributes_id . ", '" . tep_db_input($products_attributes_filename) . "', '" . $products_attributes_maxdays . "', '" . $products_attributes_maxcount . "')");
          }
        }
      ...
      case 'update_value':
        ...
        for ($i=0, $n=sizeof($languages); $i<$n; $i ++) {
          ...
          tep_db_query("update " . TABLE_PRODUCTS_OPTIONS_VALUES . " set products_options_values_name = '" . tep_db_input($value_name) . "' where products_options_values_id = '" . $value_id . "' and language_id = '" . (int)$languages[$i]['id'] . "'");
        }

        tep_db_query("update " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . " set products_options_id = '" . $option_id . "'  where products_options_values_id = '" . $value_id . "'");
      ...
      case 'update_product_attribute':
        ...
        $value_price = max(0, floatval(str_replace(',', '.', xprios_prepare_post('value_price'))));
        $price_prefix = xprios_prepare_post('price_prefix');
        $attribute_id = intval(xprios_prepare_post('attribute_id'));

        tep_db_query("update " . TABLE_PRODUCTS_ATTRIBUTES . " set products_id = '" . $products_id . "', options_id = '" . $options_id . "', options_values_id = '" . $values_id . "', options_values_price = '" . tep_db_input($value_price) . "', price_prefix = '" . tep_db_input($price_prefix) . "' where products_attributes_id = '" . $attribute_id . "'");
        ...
        if (DOWNLOAD_ENABLED == 'true') {
          ...
          if (tep_not_null($products_attributes_filename)) {
            tep_db_query("replace into " . TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " set products_attributes_id = '" . $attribute_id . "', products_attributes_filename = '" . tep_db_input($products_attributes_filename) . "', products_attributes_maxdays = '" . $products_attributes_maxdays . "', products_attributes_maxcount = '" . $products_attributes_maxcount . "'");
          }
        }
      ...
      case 'delete_option':
        $option_id = intval(xprios_prepare_get('option_id'));

        tep_db_query("delete from " . TABLE_PRODUCTS_OPTIONS . " where products_options_id = '" . $option_id . "'");
      ...
      case 'delete_value':
        ...
        tep_db_query("delete from " . TABLE_PRODUCTS_OPTIONS_VALUES . " where products_options_values_id = '" . $value_id . "'");
        tep_db_query("delete from " . TABLE_PRODUCTS_OPTIONS_VALUES_TO_PRODUCTS_OPTIONS . " where products_options_values_id = '" . $value_id . "'");
      ...
      case 'delete_attribute':
        ...
        tep_db_query("delete from " . TABLE_PRODUCTS_ATTRIBUTES . " where products_attributes_id = '" . $attribute_id . "'");

// added for DOWNLOAD_ENABLED. Always try to remove attributes, even if downloads are no longer enabled
        tep_db_query("delete from " . TABLE_PRODUCTS_ATTRIBUTES_DOWNLOAD . " where products_attributes_id = '" . $attribute_id . "'");
            ...
            <td colspan="5">
              <table>
                <tr class="<?php echo (!($rows % 2)? 'attributes-even' : 'attributes-odd');?>">
                  <td class="dataTableHeadingContent"><?php echo TABLE_HEADING_DOWNLOAD; ?>&nbsp;</td>
                  <td class="smallText"><?php echo TABLE_TEXT_FILENAME; ?></td>
                  <td class="smallText"><?php echo tep_draw_input_field('products_attributes_filename', '', 'size="25"'); ?>&nbsp;</td>
                  <td class="smallText"><?php echo TABLE_TEXT_MAX_DAYS; ?></td>
                  <td class="smallText"><?php echo tep_draw_input_field('products_attributes_maxdays', $products_attributes_maxdays, 'size="5"'); ?>&nbsp;</td>
                  <td class="smallText"><?php echo TABLE_TEXT_MAX_COUNT; ?></td>
                  <td class="smallText"><?php echo tep_draw_input_field('products_attributes_maxcount', $products_attributes_maxcount, 'size="5"'); ?>&nbsp;</td>
