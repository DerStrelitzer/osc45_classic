    ...
    switch ($_POST['action']) {
      ...
      case 'products_image_upload_med':
        if (DREI_PRODUKTBILDER == true) {
            $products_image = new Upload('products_image', DIR_FS_CATALOG_IMAGES_MEDIUM);
            if ($products_image->parse() && $products_image->save())  {
                $code = 0;
                $message = 'OK';
            } else {
      ...
      case 'products_image_upload_large':
        if (DREI_PRODUKTBILDER == true) {
            ...
            $products_image = new Upload('products_image', DIR_FS_CATALOG_IMAGES_LARGE);
            if ($products_image->parse() && $products_image->save())  {
                $code = 0;
                $message = 'OK';
            } else {
      ...
      case 'manufacturers_update':
        ...
        $manufacturers_id = xprios_prepare_post('mID');
        ...
        if (isset($manufacturers_id))
        {
	        ...
	        if (isset($_POST['manufacturers_name'])) $manufacturers_name = xprios_prepare_post('manufacturers_name');
	        if (isset($_POST['manufacturers_image'])) $manufacturers_image = xprios_prepare_post('manufacturers_image');
            ...
            $post_manufacturers_url = xprios_prepare_post('manufacturers_url');
            $post_url_clicked       = xprios_prepare_post('url_clicked');
            $post_date_last_click   = xprios_prepare_post('date_last_click');
	        ...
	        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
	          ...
	          // uebergebene Daten einsetzen
                if (isset($post_manufacturers_url[$language_id])) {
                    $manufacturers_url = $post_manufacturers_url[$language_id];
                }
                if (isset($post_url_clicked[$language_id])) {
                    $url_clicked = $post_url_clicked[$language_id];
                }
                if (isset($post_date_last_click[$language_id])) {
                    $date_last_click = $post_date_last_click[$language_id];
                }
      ...
      case 'products_update':
        ...
        $products_id = xprios_prepare_post('pID');
        ...
        // Variablen nur ueberschreiben wenn als Parameter vorhanden !!!
        if (isset($_POST['products_quantity'])) $products_quantity = xprios_prepare_post('products_quantity');
        if (isset($_POST['products_model'])) $products_model = xprios_prepare_post('products_model');
        if (isset($_POST['products_image'])) $products_image = xprios_prepare_post('products_image');

        if (DREI_PRODUKTBILDER == true)
        {
          if (isset($_POST['products_image_med'])) $products_image_med = xprios_prepare_post('products_image_med');
          if (isset($_POST['products_image_large'])) $products_image_large = xprios_prepare_post('products_image_large');
        }

        if (isset($_POST['products_price'])) $products_price = xprios_prepare_post('products_price');
        if (isset($_POST['products_date_available'])) $products_date_available = xprios_prepare_post('products_date_available');
        if (isset($_POST['products_weight'])) $products_weight = xprios_prepare_post('products_weight');
        if (isset($_POST['products_status'])) $products_status = xprios_prepare_post('products_status');
        if (isset($_POST['products_tax_class_id'])) $products_tax_class_id = xprios_prepare_post('products_tax_class_id');
        if (isset($_POST['manufacturers_id'])) $manufacturers_id = xprios_prepare_post('manufacturers_id');
        ...
        $post_products_name = xprios_prepare_post('products_name');
        $post_products_description = xprios_prepare_post('products_description', true);
        $post_$products_url = xprios_prepare_post('products_url');
        $languages = tep_get_languages();
        for ($i = 0, $n = sizeof($languages); $i < $n; $i++) {
            $language_id = $languages[$i]['id'];
          ...
          // uebergebene Daten einsetzen
            if (isset($post_products_name[$language_id])) {
                $products_name = $post_products_name[$language_id];
            }
            if (isset($post_products_description[$language_id])) {
                $products_description = $post_products_description[$language_id];
            }
            if (isset($post_$products_url[$language_id])) {
                $products_url = $post_$products_url[$language_id];
            }
        ...
        $ProdID  = xprios_prepare_post('prodid');
		...
		case 'categories_update':
		  ...
		  $CatID    = xprios_prepare_post('catid']);
		  $ParentID = xprios_prepare_post('parentid']);
		  $Sort     = xprios_prepare_post('sort']);
		  $Image    = xprios_prepare_post('image']);
		  $Name     = urldecode(xprios_prepare_post('name'));
        ...
        if ((isset($_POST['order_id'])) && (isset($_POST['status'])))
        {
          ...
          $oID = xprios_prepare_post('order_id');
          $status = xprios_prepare_post('status');
          $comments = xprios_prepare_post('comments');
          $cao_language = xprios_prepare_post('cao_language');  // german, english, espanol...
            ...
            if ($check_status['orders_status'] != $status || $comments != '')
            {
              ...
              if ($_POST['notify'] == 'on')
              {
                ...
                if (file_exists(DIR_WS_LANGUAGES . $cao_language . '/' . FILENAME_ORDERS))
                {
                  ...
                  $_SESSION['language'] = $cao_language;
                } else if (file_exists(DIR_WS_LANGUAGES . $_SESSION['language'] . '/' . FILENAME_ORDERS))
                {
                  include(DIR_WS_LANGUAGES . $_SESSION['language'] . '/' . FILENAME_ORDERS);
                }
      ...
      //-- Raphael Vullriede
		...
		case 'customers_update':
			$customers_id = -1;
			if (isset($_POST['cID'])) $customers_id = xprios_prepare_post('cID');
...
// JAN				$address_id = tep_db_insert_id();
// JAN				tep_db_query("update " . TABLE_CUSTOMERS . " set customers_default_address_id = '" . (int)$address_id . "' where customers_id = '" . (int)$customers_id . "'");
            tep_db_query("update " . TABLE_CUSTOMERS . " set customers_default_address_id = 1 where customers_id = '" . (int)$customers_id . "'");
  	...
  	else
  {
    ...
    if (($_GET['action']) && ($REQUEST_METHOD=='GET') && ($_GET['action']))
    {
      $schema = '<?xml version="1.0" encoding="' . CHARSET . '"?>' . "\n" .
                '<STATUS>' . "\n" .
                '<STATUS_DATA>' . "\n" .
