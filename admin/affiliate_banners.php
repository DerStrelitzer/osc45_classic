<?php
/*
  $Id: affiliate_banners.php,v 1.6 2003/07/12 13:03:21 simarilius Exp $
...
switch ($action) {
    case 'setaffiliate_flag':
        ...
        tep_redirect(tep_href_link(CURRENT_PAGE, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'abID=' . $_GET['abID']));
    break;
    
    case 'insert':
    case 'update':
        $affiliate_banners_id = xprios_prepare_post('affiliate_banners_id');
        $affiliate_banners_title = xprios_prepare_post('affiliate_banners_title');
        $affiliate_products_id  = xprios_prepare_post('affiliate_products_id');
        $new_affiliate_banners_group = xprios_prepare_post('new_affiliate_banners_group');
        $affiliate_banners_group = (empty($new_affiliate_banners_group)) ? xprios_prepare_post('affiliate_banners_group') : $new_affiliate_banners_group;
        $affiliate_banners_image_target = xprios_prepare_post('affiliate_banners_image_target');
        $affiliate_html_text = xprios_prepare_post('affiliate_html_text');
        $affiliate_banners_image_local = xprios_prepare_post('affiliate_banners_image_local');
        $affiliate_banners_image_target = xprios_prepare_post('affiliate_banners_image_target');
        $db_image_location = '';
...
*/
        ...
        if (!$affiliate_banner_error) {
            ...
            if ($action == 'insert') {
                $sql_data_array['affiliate_status'] = '1';
        ...
        // Banner ID 1 is generic Product Banner
            ...
            tep_redirect(tep_href_link(CURRENT_PAGE, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . '&abID=' . $affiliate_banners_id));
        } else {
            $action = 'new';
        }
    break;
    ...
    case 'deleteconfirm':
        $affiliate_banners_id = xprios_prepare_get('abID');
        $delete_image = xprios_prepare_post('delete_image');
        ...
        tep_redirect(tep_href_link(CURRENT_PAGE, (isset($_GET['page']) ? 'page=' . $_GET['page'] : '')));
        break;
}
...
?>
      ...
      <tr>
        <td><table border="0" width="100%" cellspacing="0" cellpadding="2">
          <tr>
            <td class="main" align="right" valign="top" nowrap><?php 
    echo (($form_action == 'insert') ? tep_image_submit('button_insert.gif', IMAGE_INSERT) : tep_image_submit('button_update.gif', IMAGE_UPDATE))
          . '&nbsp;&nbsp;<a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, (isset($_GET['page']) ? 'page=' . $_GET['page'] . '&' : '') . 'abID=' . $_GET['abID']) . '">' 
...
<?php
    ...
    switch ($action) {
        case 'delete':
            ...
            $contents[] = array('align' => 'center', 'text' => '<br>' . tep_image_submit('button_delete.gif', IMAGE_DELETE) . '&nbsp;<a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'page=' . $page . '&abID=' . $_GET['abID']) . '">' . tep_image_button('button_cancel.gif', IMAGE_CANCEL) . '</a>');
        break;

        default:
            if (isset($abInfo) && is_object($abInfo)) {
                $sql = "select products_name from " . TABLE_PRODUCTS_DESCRIPTION . " where products_id = '" . $abInfo->affiliate_products_id . "' and language_id = '" . (int)$_SESSION['languages_id'] . "'";
                $product_description_query = tep_db_query($sql);
                $product_description = tep_db_fetch_array($product_description_query);
                $heading[] = array('text' => '<b>' . $abInfo->affiliate_banners_title . '</b>');

                $contents[] = array('align' => 'center', 'text' => '<a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'page=' . $page . '&abID=' . $abInfo->affiliate_banners_id . '&action=new') . '">' . tep_image_button('button_edit.gif', IMAGE_EDIT) . '</a> <a href="' . tep_href_link(FILENAME_AFFILIATE_BANNER_MANAGER, 'page=' . $page . '&abID=' . $abInfo->affiliate_banners_id . '&action=delete') . '">' . tep_image_button('button_delete.gif', IMAGE_DELETE) . '</a>');
