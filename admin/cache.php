  ...
  if ($messageStack->size < 1) {
    ...
    for ($i=0, $n=sizeof($languages); $i<$n; $i++) {
      if ($languages[$i]['code'] == DEFAULT_LANGUAGE) {
        $this_language = $languages[$i]['directory'];
      }
    }

    for ($i=0, $n=sizeof($cache_blocks); $i<$n; $i++) {
      $cached_file = preg_replace('/-language/', '-' . $this_language, $cache_blocks[$i]['file']);

      if (file_exists(DIR_FS_CACHE . $cached_file)) {
        $cache_mtime = date('Y-m-d H:i:s', filemtime(DIR_FS_CACHE . $cached_file));
      } else {
        ...
        while ($cache_file = $dir->read()) {
          ...
          if (preg_match('/^' . preg_quote($cached_file, '/') . '/', $cache_file)) {
            $cache_mtime = date('Y-m-d H:i:s', filemtime(DIR_FS_CACHE . $cache_file));
            break;
          }
        }
