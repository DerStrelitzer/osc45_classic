...
if (isset($_GET['reviews_id']) && tep_not_null($_GET['reviews_id']) && isset($_GET['products_id']) && tep_not_null($_GET['products_id'])) {
    $review_check_query = tep_db_query("select count(*) as total from " . TABLE_REVIEWS . " r, " . TABLE_REVIEWS_DESCRIPTION . " rd where r.reviews_id = '" . (int)$_GET['reviews_id'] . "' and r.products_id = '" . (int)$_GET['products_id'] . "' and r.reviews_id = rd.reviews_id and rd.languages_id = '" . (int)$_SESSION['languages_id'] . "'");
...
$this_products_tax_rate = tep_get_tax_rate($review['products_tax_class_id']);

if ($new_price = tep_get_products_special_price($review['products_id'])) {
    $products_price = '<span class="striked">' . $currencies->display_price($review['products_price'], $this_products_tax_rate) . '</span> <span class="productSpecialPrice">' . $currencies->display_price($new_price, $this_products_tax_rate) . '</span>';
} else {
    $products_price = $currencies->display_price($review['products_price'], $this_products_tax_rate);
}
$products_price .= ingo_price_added($this_products_tax_rate);
  
$products_name = sprintf(HEADING_TITLE_RAW, $review['products_name']);
if (tep_not_null($review['products_model'])) {
    $products_name .= '<br /><span class="smallText">[' . $review['products_model'] . ']</span>';
}
                ...
                <td>
                  <div class="divbox">
                    <div style="margin:2px 12px">
                        <div><?php echo tep_break_string(nl2br(tep_output_string_protected($review['reviews_text'])), 60, '-<br />') . '<br /><br /><i>' . sprintf(TEXT_REVIEW_RATING, tep_image(DIR_WS_IMAGES . 'desk/stars_' . $review['reviews_rating'] . '.gif', sprintf(TEXT_OF_5_STARS, $review['reviews_rating'])), sprintf(TEXT_OF_5_STARS, $review['reviews_rating'])) . '</i>'; ?></div>
                    </div>
                  </div>
                </td>
              </tr>
                ...
                <td>
                  <div class="divbox">
                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                      <tr>
                        <td class="main"><?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id'))) . '">' . tep_image_button('button_back.gif', IMAGE_BUTTON_BACK, 'style="margin-left:10px;"') . '</a>'; ?></td>
                        <td class="main" align="right"><?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, tep_get_all_get_params(array('reviews_id'))) . '">' . tep_image_button('button_write_review.gif', IMAGE_BUTTON_WRITE_REVIEW, 'style="margin-right:10px;"') . '</a>'; ?></td>
                      </tr>
                    </table>
                  </div>
                </td>
...
<noscript>
<?php 
    echo '<a href="' . tep_href_link(DIR_WS_IMAGES . $review['products_image']) . '" target="_blank">' . tep_image(DIR_WS_IMAGES . $review['products_image'], $review['products_name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT, 'hspace="5" vspace="5"') . '<br />' . TEXT_CLICK_TO_ENLARGE . '</a>'; 
?>
</noscript>
<?php
}

echo '<p><a href="' . tep_href_link(basename($PHP_SELF), tep_get_all_get_params(array('action')) . 'action=buy_now') . '">' . tep_image_button('button_in_cart.gif', IMAGE_BUTTON_IN_CART, 'style="margin-left:10px;"') . '</a></p>';
?>
                </td>
              </tr>
            </table>
