...
function tep_cache_categories_box($auto_expire = false, $refresh = false)
{
    ...
    if (($refresh == true) || !read_cache($cache_output, 'categories_box-' . $GLOBALS['language'] . '.cache' . $cPath, $auto_expire)) {
        ob_start();
        include(DIR_WS_BOXES . 'categories.php');
        $cache_output = ob_get_contents();
        ob_end_clean();
        write_cache($cache_output, 'categories_box-' . $GLOBALS['language'] . '.cache' . $cPath);
    }
...
function tep_cache_manufacturers_box($auto_expire = false, $refresh = false)
{
    ...
    if ($refresh == true || !read_cache($cache_output, 'manufacturers_box-' . $GLOBALS['language'] . '.cache' . $manufacturers_id, $auto_expire)) {
        ob_start();
        include(DIR_WS_BOXES . 'manufacturers.php');
        $cache_output = ob_get_contents();
        ob_end_clean();
        write_cache($cache_output, 'manufacturers_box-' . $GLOBALS['language'] . '.cache' . $manufacturers_id);
    }
...
function tep_cache_also_purchased($auto_expire = false, $refresh = false)
{
    ...
    if (($refresh == true) || !read_cache($cache_output, 'also_purchased-' . $GLOBALS['language'] . '.cache' . $_GET['products_id'], $auto_expire)) {
        ob_start();
        include(DIR_WS_MODULES . FILENAME_ALSO_PURCHASED_PRODUCTS);
        $cache_output = ob_get_contents();
        ob_end_clean();
        write_cache($cache_output, 'also_purchased-' . $GLOBALS['language'] . '.cache' . $_GET['products_id']);
    }
