...
{
    $banners_query = tep_db_query("select banners_id, date_scheduled from " . TABLE_BANNERS . " where date_scheduled < CURRENT_TIME");
...
function tep_expire_banners()
{
    ...
    if (tep_db_num_rows($banners_query)) {
        while ($banners = tep_db_fetch_array($banners_query)) {
            ...
            } elseif ($banners['expires_impressions'] > 0 && $banners['banners_shown'] >= $banners['expires_impressions']) {
                tep_set_banner_status($banners['banners_id'], '0');
            }
        }
    }
