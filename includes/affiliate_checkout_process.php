<?php
/*
  $Id: affiliate_checkout_process.php,v 1.9 2003/07/12 11:48:00 simarilius Exp $
...
if (isset($_SESSION['affiliate_ref']) && AFFILATE_INDIVIDUAL_PERCENTAGE == 'true') {
    $affiliate_commission_query = tep_db_query('select affiliate_commission_percent from ' . TABLE_AFFILIATE . ' where affiliate_id = ' . intval($_SESSION['affiliate_ref']));
...
if (isset($_SESSION['affiliate_ref'])) {
    $sql_data_array = [
        'affiliate_id' => intval($_SESSION['affiliate_ref']),
        ...
        'affiliate_clickthroughs_id' => intval($_SESSION['affiliate_clickthroughs_id']),
        'affiliate_percent' => $affiliate_percent,
        'affiliate_salesman' => intval($_SESSION['affiliate_ref'])
    ];
    tep_db_perform(TABLE_AFFILIATE_SALES, $sql_data_array);

    if (AFFILATE_USE_TIER == 'true') {
        $affiliate_tiers_query = tep_db_query ("SELECT "
            . "aa2.affiliate_id, (aa2.affiliate_rgt - aa2.affiliate_lft) as height "
            ...
            . "AND aa1.affiliate_id =  '" . intval($_SESSION['affiliate_ref']) . " "
            . "ORDER by height asc limit 1, " . AFFILIATE_TIER_LEVELS
        );
        $affiliate_tier_percentage = preg_split('/[;]/' , AFFILIATE_TIER_PERCENTAGE, -1, PREG_SPLIT_NO_EMPTY);
        $i=0;
