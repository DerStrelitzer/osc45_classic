...
class ups extends ShippingModules
{
...
// class constructor
    public function __construct() {
        ...
        $this->sort_order = defined('MODULE_SHIPPING_UPS_SORT_ORDER') ? MODULE_SHIPPING_UPS_SORT_ORDER : 0;
        $this->icon = DIR_WS_ICONS . 'shipping_ups.gif';
        $this->tax_class = defined('MODULE_SHIPPING_UPS_TAX_CLASS') ? MODULE_SHIPPING_UPS_TAX_CLASS : 0;
    ...
    public function quote($method = '') {
        ...
        if ( is_array($upsQuote) && sizeof($upsQuote) > 0 ) {
            ...
            for ($i=0; $i<$qsize; $i++) {
                ...
                $this_ups_costs = ($cost + MODULE_SHIPPING_UPS_HANDLING) * $shipping_num_boxes;
            // eingetragener Wert ist inklusive Steuer, sofern Warenkorb besteuert ist
                $this_ups_costs = $this_ups_costs /(100+$order->tax_max['rate'])*100;
                $methods[] = [
                    'id'    => $type,
...
*/
    ...
    public function install()
    {
        ...
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_UPS_TAX_CLASS', '0', '<b>unbenutzt!</b> Es wird der höchste Steuersatz aus dem Warenkorb benutzt.<!-- Use the following tax class on the shipping fee.-->', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
    ...
    private function _upsWeight($foo)
    {
