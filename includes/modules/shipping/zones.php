...
class zones extends ShippingModules
{
    public 
        $code = 'zones';
    private
...
// class constructor
    public function __construct()
    {
        ...
        $this->sort_order = defined('MODULE_SHIPPING_ZONES_SORT') ? ORDERMODULE_SHIPPING_ZONES_SORT_ORDER : 0;
        $this->icon = '';
        $this->tax_class = defined('MODULE_SHIPPING_ZONES_TAX_CLASS') ? MODULE_SHIPPING_ZONES_TAX_CLASS : 0;
        $this->enabled = defined('MODULE_SHIPPING_ZONES_STATUS') && MODULE_SHIPPING_ZONES_STATUS == 'ja' ? true : false;
...
// class methods
    public function quote($method = '') 
    {
        ...
        for ($i=1; $i<=$this->num_zones; $i++) {
            ...
            $country_zones = preg_split('/[,]/', $countries_table, -1, PREG_SPLIT_NO_EMPTY);
        ...
        if ($dest_zone == 0) {
            ...
            $zones_table = preg_split('/[:,]/' , $zones_cost, -1, PREG_SPLIT_NO_EMPTY);
    ...
    public function install() {
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) VALUES ('Enable Zones Method', 'MODULE_SHIPPING_ZONES_STATUS', 'ja', 'Do you want to offer zone rate shipping?', '6', '0', 'tep_cfg_select_option(array(\'ja\', \'nein\'), ', now())");
        tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Tax Class', 'MODULE_SHIPPING_ZONES_TAX_CLASS', '0', '<b>unbenutzt!</b> Es wird der höchste Steuersatz aus dem Warenkorb benutzt.<!-- Use the following tax class on the shipping fee.-->', '6', '0', 'tep_get_tax_class_title', 'tep_cfg_pull_down_tax_classes(', now())");
