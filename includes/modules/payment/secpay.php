...
class secpay extends PaymentModules
{
    public 
      $code = 'secpay';
...
// class constructor
    public function __construct() {
        ...
        $this->sort_order = defined('MODULE_PAYMENT_SECPAY_SORT_ORDER') ? MODULE_PAYMENT_SECPAY_SORT_ORDER : 0;
        $this->enabled = defined('MODULE_PAYMENT_SECPAY_STATUS') && MODULE_PAYMENT_SECPAY_STATUS == 'ja' ? true : false;

        if (defined('MODULE_PAYMENT_SECPAY_ORDER_STATUS_ID') && (int)MODULE_PAYMENT_SECPAY_ORDER_STATUS_ID > 0) {
...
// class methods
    ...
    function install() {
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('SECpay', 'MODULE_PAYMENT_SECPAY_STATUS', 'ja', 'Wird die Zahlung durch SECPay angeboten?', '6', '1', 'tep_cfg_select_option(array(\'ja\', \'nein\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Handels ID', 'MODULE_PAYMENT_SECPAY_MERCHANT_ID', 'secpay', 'Hier wird die Handels ID eingetragen, die mit dem SECPay Service benutzt wird.', '6', '2', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Transaktionswährung', 'MODULE_PAYMENT_SECPAY_CURRENCY', 'Any Currency', 'Die Währung, mit der die Kreditkartentransaktionen hier erbeiten.<br /><b>any</b> = jede<br /><b>default</b> = Standardwährung', '6', '3', 'tep_cfg_select_option(array(\'Any Currency\', \'Default Currency\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, date_added) values ('Transaktionsmodus', 'MODULE_PAYMENT_SECPAY_TEST_STATUS', 'Always Successful', 'Hier wird der Transactionsmodus zur Benutzung mit dem SECPay Service eingestellt.', '6', '4', 'tep_cfg_select_option(array(\'Always Successful\', \'Always Fail\', \'Production\'), ', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, date_added) values ('Sortierreihenfolge', 'MODULE_PAYMENT_SECPAY_SORT_ORDER', '0', 'Reihenfolge der Anzeige. Niedrigste zuerst.<center>_______________________<br />| deutsche Übersetzung |<br />|_____<a href=\"mailto:videoundco@web.de?subject=Frage zu osCommerce\"><b><u>In</u>g<u>o Malchow</u></b></a>____|</center> ', '6', '0', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, use_function, set_function, date_added) values ('Zone', 'MODULE_PAYMENT_SECPAY_ZONE', '0', 'Wird hier eine Zone ausgewählt, wird die Zahlungsart nur für Aufträge aus dieser Zone möglich. ', '6', '2', 'tep_get_zone_class_title', 'tep_cfg_pull_down_zone_classes(', now())");
      tep_db_query("insert into " . TABLE_CONFIGURATION . " (configuration_title, configuration_key, configuration_value, configuration_description, configuration_group_id, sort_order, set_function, use_function, date_added) values ('Bestellstatus', 'MODULE_PAYMENT_SECPAY_ORDER_STATUS_ID', '0', 'Wird eine Bestellung ausgelöst, so wird dieser, die hier eingetragene Stufe des Bestellstatus zugewiesen.', '6', '0', 'tep_cfg_pull_down_order_statuses(', 'tep_get_order_status_name', now())");
    }
