...
$expected_query = tep_db_query("select p.products_id, pd.products_name, products_date_available as date_expected from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where to_days(products_date_available) >= to_days(now()) and p.products_id = pd.products_id and pd.language_id = '" . (int)$GLOBALS['languages_id'] . "' order by " . EXPECTED_PRODUCTS_FIELD . " " . EXPECTED_PRODUCTS_SORT . " limit " . MAX_DISPLAY_UPCOMING_PRODUCTS);
...
<?php
    ...
    while ($expected = tep_db_fetch_array($expected_query)) {
        ...
        echo '                <td class="smallText">&nbsp;<a href="' . ingo_product_link($expected['products_id'], $expected['products_name']) . '">' . $expected['products_name'] . '</a>&nbsp;</td>' . "\n"
