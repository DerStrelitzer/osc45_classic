<?php
/*
  $Id: HttpClient.php,v 1.1 2002/11/01 02:26:03 hpdl Exp $
...
{
    public 
        $url, // array containg server URL, similar to parseurl() returned array
        $reply, // response code
        $replyString, // full response
        $protocolVersion = '1.1',
 ...
 **/
    public function processHeader($lastLine = "\r\n")
    {
        ...
        while (!$finished && !feof($this->socket)) {
            ...
            if (!$finished) {
                list($hdr, $value) = preg_split('/: /', $str, 2, PREG_SPLIT_NO_EMPTY);
