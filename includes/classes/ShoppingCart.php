<?php
/*
  $Id: ShoppingCart.php,v 1.35 2003/06/25 21:14:33 hpdl Exp $
...
class ShoppingCart
{
    ...
    public function reset($reset_database = false)
    {
        ...
        unset($this->cartID, $_SESSION['cartID']);
    }
...
// insert current cart contents in database
        if (is_array($this->contents)) {
            foreach (array_keys($this->contents) as $products_id) {
                ...
                if (!tep_db_num_rows($product_query) && $_SESSION['customer_id']>0) {
...
// insert into database
                            if (isset($_SESSION['customer_id']) && $_SESSION['customer_id']>0) tep_db_query("insert into " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " (customers_id, products_id, products_options_id, products_options_value_id) values ('" . (int)$_SESSION['customer_id'] . "', '" . tep_db_input($products_id_string) . "', '" . (int)$option . "', '" . (int)$value . "')");
                        }
                    }
                }
...
// update database
            if (isset($_SESSION['customer_id']) && $_SESSION['customer_id']>0) {
                tep_db_query("update " . TABLE_CUSTOMERS_BASKET . " set customers_basket_quantity = '" . (int)$quantity . "' where customers_id = '" . (int)$_SESSION['customer_id'] . "' and products_id = '" . tep_db_input($products_id_string) . "'");
            }
            ...
            if (is_array($attributes)) {
                foreach ($attributes as $option => $value) {
                    $this->contents[$products_id_string]['attributes'][$option] = $value;
// update database
                    if (isset($_SESSION['customer_id']) && $_SESSION['customer_id']>0) tep_db_query("update " . TABLE_CUSTOMERS_BASKET_ATTRIBUTES . " set products_options_value_id = '" . (int)$value . "' where customers_id = '" . (int)$_SESSION['customer_id'] . "' and products_id = '" . tep_db_input($products_id_string) . "' and products_options_id = '" . (int)$option . "'");
                }
            }
        }
    }
...
// assign a temporary unique ID to the order contents to prevent hack attempts during the checkout procedure
    ...
    public function get_product_id_list()
    {
        ...
        if (is_array($this->contents)) {
            foreach (array_keys($this->contents) as $products_id) {
                $product_id_list .= ($product_id_list=='' ? '':', ') . $products_id;
            }
        }
...
// products price
            ...
            if ($product = tep_db_fetch_array($product_query)) {
                ...
                tep_db_free_result($specials_query);
...
// attributes price
            if (isset($this->contents[$products_id]['attributes'])) {
                foreach ($this->contents[$products_id]['attributes'] as $option => $value) {
                    ...
                    tep_db_free_result($attribute_price_query);
                }
            }
        }
    }
    ...
    public function get_products()
    {
        ...
        $products_array = [];
        foreach (array_keys($this->contents) as $products_id) {
            $products_query = tep_db_query("select p.products_id, pd.products_name, p.products_model, p.products_image, p.products_price, p.products_weight, p.products_tax_class_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$products_id . "' and pd.products_id = p.products_id and pd.language_id = '" . (int)$GLOBALS['languages_id'] . "'");
            ...
            foreach (array_keys($this->contents) as $products_id) {
                if (isset($this->contents[$products_id]['attributes'])) {
                    foreach ($this->contents[$products_id]['attributes'] as $value) {
                        ...
                        tep_db_free_result($virtual_check_query);
                    }
                } else {
                    switch ($this->content_type) {
                        case 'virtual':
