...
if (isset($_GET['action']) && ($_GET['action'] == 'process')) {
    $rating = xprios_prepare_post('rating');
    $review = xprios_prepare_post('review');
    ...
    if ($error == false) {
        tep_db_query("insert into " . TABLE_REVIEWS . " (products_id, customers_id, customers_name, reviews_rating) values ('" . (int)$_GET['products_id'] . "', '" . (int)$_SESSION['customer_id'] . "', '" . tep_db_input($customer['customers_firstname']) . ' ' . tep_db_input($customer['customers_lastname']) . "', '" . tep_db_input($rating) . "')");
...
if ($new_price = tep_get_products_special_price($product_info['products_id'])) {
    $products_price = '<span class="striked">' . $currencies->display_price($product_info['products_price'], $this_products_tax_rate) . '</s> <span class="productSpecialPrice">' . $currencies->display_price($new_price, $this_products_tax_rate) . '</span>';
} else {
...
$this_content_form = tep_draw_form('product_reviews_write', tep_href_link(FILENAME_PRODUCT_REVIEWS_WRITE, 'action=process&products_id=' . $_GET['products_id']), 'post', 'onsubmit="return checkForm();"');
                ...
                <td>
                  <div class="divbox">
                    <table border="0" width="100%" cellspacing="0" cellpadding="2">
                      <tr>
                        <td class="main"><?php echo '<a href="' . tep_href_link(FILENAME_PRODUCT_REVIEWS, tep_get_all_get_params(array('reviews_id', 'action'))) . '">' . tep_image_button('button_back.gif', IMAGE_BUTTON_BACK, 'style="margin-left:10px;"') . '</a>'; ?></td>
