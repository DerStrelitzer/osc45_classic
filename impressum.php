<?php
/*
  $Id: impressum.php,v 1.00 2002/12/16 11:14:00 harley_vb Exp $
...
if (!@define('CURRENT_PAGE', basename(__FILE__))) {
  ...
  $breadcrumb->add(NAVBAR_TITLE, tep_href_link(FILENAME_IMPRESSUM, '', 'NONSSL'));

  if (defined('READ_INFO_TEXTE_FROM_DATABASE') && READ_INFO_TEXTE_FROM_DATABASE=='ja') {
    $info_text = '';
    $text_query = tep_db_query('SELECT text FROM ' . TABLE_INFO_TEXTE . ' WHERE code = "impressum" AND languages_id = ' . (int)$_SESSION['languages_id']);
    if ($text_result = tep_db_fetch_array($text_query)) {
      $info_text = trim($text_result['text']);
    }
  } else {
    $info_text = TEXT_INFORMATION . '<br />' . DISCLAIMER;
  }
