...
if (isset($_POST['action'])) {
    ...
    $check_email_query = tep_db_query('select count(*) as total from ' . TABLE_AFFILIATE . ' where affiliate_email_address = "' .  tep_db_input($a_email_address) . '" and affiliate_id != ' . intval($_SESSION['affiliate_id']));
    ...
    if (!$error) {
        ...
        tep_db_perform(TABLE_AFFILIATE, $sql_data_array, 'update', 'affiliate_id = ' . intval($_SESSION['affiliate_id']));
...
$this_content_form = tep_draw_form('affiliate_details', tep_href_link(FILENAME_AFFILIATE_DETAILS, '', 'SSL'), 'post') . tep_draw_hidden_field('action', 'process');
...
?>
      ...
      <tr>
        <td>
<?php
$affiliate_query = tep_db_query('select * from ' . TABLE_AFFILIATE . ' where affiliate_id = ' . intval($_SESSION['affiliate_id']));
