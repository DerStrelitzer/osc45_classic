<?php
/*
  $Id: affiliate_show_banner.php,v 1.13 2003/02/28 10:27:13 simarilius Exp $
...
?>
    <table border=1 cellpadding=2 cellspacing=2>
      ...
      <tr><td>absolute path to picture:</td>
          <td><?php echo DIR_FS_DOCUMENT_ROOT . DIR_WS_CATALOG. DIR_WS_IMAGES . $banner; ?></td>
          </tr>
...
if (isset($_GET['ref'])) {
    $affiliate_id = max(0, intval($_GET['ref']));
} elseif (isset($_POST['ref'])) {
    $affiliate_id = max(0, intval($_POST['ref']));
}

if (isset($_GET['affiliate_banner_id'])) {
    $banner_id = max(0, intval($_GET['affiliate_banner_id']));
} elseif (isset($_POST['affiliate_banner_id'])) {
    $banner_id = max(0, intval($_POST['affiliate_banner_id']));
}
  
if (isset($_GET['affiliate_pbanner_id'])) {
    $prod_banner_id = max(0, intval($_GET['affiliate_pbanner_id']));
} elseif (isset($_POST['affiliate_pbanner_id'])) {
    $prod_banner_id = max(0, intval($_POST['affiliate_pbanner_id']));
}
